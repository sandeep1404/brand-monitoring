    ...
    async def scrape_data_and_analyse(self):
        ...
        async def web_analysis():
            if self.state.web_search_response:
                ...
                # Handle the web scraping response data structure
                for i, original_url in zip(self.state.web_scrape_response, web_urls):
                    self.state.web_filtered_scrape_response.append({
                        "url": original_url,  # Use the original URL from the search response
                        "markdown": i["markdown"] if "markdown" in i else ""
                    })
